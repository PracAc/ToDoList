...
function registerStorage() {
  _registerComponent(new Component(STORAGE_TYPE, factory2, "PUBLIC").setMultipleInstances(true));
...
function extractRefs(doc, oldDoc, subs, options) {
  ...
  function recursiveExtract(doc2, oldDoc2, path, result) {
    ...
    for (const key in doc2) {
      const ref2 = doc2[key];
      if (ref2 == null || ref2 instanceof Date || ref2 instanceof Timestamp || ref2 instanceof GeoPoint) {
        data[key] = ref2;
      } else if (isDocumentRef(ref2)) {
        const refSubKey = path + key;
        data[key] = refSubKey in subs ? oldDoc2[key] : ref2.path;
...
function useFirebaseAuth(name5) {
  return isClient ? getAuth(useFirebaseApp(name5)) : null;
}
